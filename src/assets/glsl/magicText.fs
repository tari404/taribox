...
void main() {
  float offset = snoise(vec3(vUv * 3.0, uTime * 0.6));
  ...
  color += uColor1 * 0.15 * aastep(threshold + 0.05, sdf + 0.5 * snoise(vec3(uv * 10.0, uTime)));
  color += uColor1 * 0.3 * aastep(threshold, sdf + 0.1 * snoise(vec3(uv * 40.0, uTime)));
